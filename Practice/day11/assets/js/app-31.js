...
const kartaYaz = (data) => {
	...
	document.getElementById("tarih").innerText = date.toLocaleDateString(
		"en-US",
		options
	);
	document.getElementById("sicaklik").innerText =
		data.current_observation.condition.temperature + `\xB0 C`;
	document.getElementById("durumYazi").innerText =
		data.current_observation.condition.text;
	document.getElementById("yerimiz").innerText = data.location.city + ", TR";

	document.getElementById("durum").innerHTML = durumResmi(
		data.current_observation.condition.code
	);
	document.getElementById("durum1").innerHTML = durumResmi(
		data.forecasts[1].code
	);
	document.getElementById("durum2").innerHTML = durumResmi(
		data.forecasts[2].code
	);
	document.getElementById("durum3").innerHTML = durumResmi(
		data.forecasts[3].code
	);
	document.getElementById("durum4").innerHTML = durumResmi(
		data.forecasts[4].code
	);
};
...
const durumResmi = (code) => {
	...
	if (code == 26 || code == 28 || code == 30)
		return '<i class="fa-solid fa-cloud"></i>';
	return '<i class="fa-solid fa-sun"></i>';
};

const fetchData = async () => {
	// const url = 'https://yahoo-weather5.p.rapidapi.com/weather?location=kayseri&format=json&u=c';
